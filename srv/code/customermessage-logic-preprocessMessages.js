 ...
 * @Before(event = { "READ" }, entity = "ingoKiesewetter_1_01Srv.CustomerMessage")
...
try {
    ...
    customerMessages = await SELECT.from('IngoKiesewetter_1_01.CustomerMessage').forUpdate();
} catch (error) {
...
await Promise.all(customerMessages.map(async customerMessage => {
    ...
    if (!titleEnglish || !messageCategory || !messageUrgency || !messageSentiment || !summaryCustomerLanguage || !summaryEnglish || !fullMessageEnglish) {
        ...
        try {
            ...
            await UPDATE('IngoKiesewetter_1_01.CustomerMessage')
