...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "ingoKiesewetter_1_01Srv.ProductFAQ")
...
try {
    ...
    productFAQ = await SELECT.one('IngoKiesewetter_1_01.ProductFAQ').where({ ID: productFAQID }).forUpdate();
    if (!productFAQ) {
...
try {
    ...
    await UPDATE('IngoKiesewetter_1_01.ProductFAQ').set({ embedding: embedding }).where({ ID });
